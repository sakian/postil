...
import { lineLabel, relativeTime, snippet } from '../format.ts';
...
function Comment({ comment, thread }: { comment: CommentView; thread: ThreadView }) {
  ...
  const { editComment, deleteComment } = useStore.getState();
  ...
  return (
    <div className={`comment comment-${comment.author}`}>
      ...
      {editing ? (
        ...
        <div className="comment-body"><Markdown text={comment.body} suggestionBase={suggestionBase} /></div>
      )}
    </div>
  );
}
...
export function ThreadWidget({ thread, showLocation = false, outdated = false }: { thread: ThreadView; showLocation?: boolean; outdated?: boolean }) {
  ...
  return (
    <div ref={ref} id={`thread-${thread.id}`} className={`thread${thread.status === 'resolved' ? ' is-resolved' : ''}${focused ? ' is-focused' : ''}`}>
      <div className="thread-head" onClick={() => setOpen((o) => !o)} role="button" aria-expanded={open}>
        ...
        {showLocation && <code className="thread-loc">{thread.path}:{lineLabel(thread)}</code>}
        {!showLocation && thread.start_line !== null && <span className="muted">{thread.side === 'old' ? 'Removed ' : ''}{lineLabel(thread)}</span>}
        {outdated && <span className="chip chip-outdated" title="The code this comment refers to has changed">Outdated</span>}
